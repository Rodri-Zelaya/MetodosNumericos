...
namespace Métodos_Numéricos
{
    public partial class FormEuler : Form
    {
        ...
        {
            // 🚀 ESTÉTICA MEJORADA: Etiquetas en Bold, Cajas de texto en Regular
            Font fontEtiqueta = new Font("Segoe UI", 11, FontStyle.Bold);
            Font fontCaja = new Font("Segoe UI", 12, FontStyle.Regular); // Letra normal para los inputs

            // Fila 1
            this.Controls.Add(new Label { Text = "Función f(x, y):", Name = "lblFuncion", AutoSize = true, Font = fontEtiqueta });
            txtFuncion.Font = fontCaja; txtFuncion.Size = new Size(280, 30);
            this.Controls.Add(txtFuncion);

            this.Controls.Add(new Label { Text = "Método a utilizar:", Name = "lblMetodo", AutoSize = true, Font = fontEtiqueta });
            cmbMetodo.Font = fontCaja; cmbMetodo.Size = new Size(240, 30);
            ...
            // Fila 2
            this.Controls.Add(new Label { Text = "X inicial (x₀):", Name = "lblX0", AutoSize = true, Font = fontEtiqueta });
            txtX0.Font = fontCaja; txtX0.Size = new Size(90, 30);
            this.Controls.Add(txtX0);

            this.Controls.Add(new Label { Text = "Y inicial (y₀):", Name = "lblY0", AutoSize = true, Font = fontEtiqueta });
            txtY0.Font = fontCaja; txtY0.Size = new Size(90, 30);
            this.Controls.Add(txtY0);

            this.Controls.Add(new Label { Text = "Valor a estimar (x_f):", Name = "lblXf", AutoSize = true, Font = fontEtiqueta });
            txtXf.Font = fontCaja; txtXf.Size = new Size(90, 30);
            this.Controls.Add(txtXf);

            this.Controls.Add(new Label { Text = "Paso (h):", Name = "lblH", AutoSize = true, Font = fontEtiqueta });
            txtH.Font = fontCaja; txtH.Size = new Size(90, 30);
        ...
        private void btnCalcular_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(txtFuncion.Text) || string.IsNullOrWhiteSpace(txtX0.Text) ||
                    string.IsNullOrWhiteSpace(txtY0.Text) || string.IsNullOrWhiteSpace(txtXf.Text) || string.IsNullOrWhiteSpace(txtH.Text))
                {
        ...
        {
            txtFuncion.Clear(); txtX0.Clear(); txtY0.Clear(); txtXf.Clear(); txtH.Clear();
            cmbMetodo.SelectedIndex = 0;
        ...
        private void AcomodarControles()
        {
            ...
            int fila1Y = 40;
            int fila2Y = 120; // 🚀 Bajé un poquito la fila 2 para que no se vea aplastada con la 1

            // Fila 1
            MoverControl("lblFuncion", 40, fila1Y - 25);
            txtFuncion.Location = new Point(40, fila1Y);

            MoverControl("lblMetodo", 360, fila1Y - 25);
            cmbMetodo.Location = new Point(360, fila1Y);

            // 🚀 ARREGLO DEL CHOQUE: Ahora usamos columnas fijas para asegurar el espacio
            int col1 = 40;
            int col2 = 180;
            int col3 = 320;
            int col4 = 550; // ¡Aquí está la magia! Le damos 230 píxeles de espacio a la etiqueta larga

            txtX0.Location = new Point(col1, fila2Y);
            MoverControl("lblX0", col1, fila2Y - 25);

            txtY0.Location = new Point(col2, fila2Y);
            MoverControl("lblY0", col2, fila2Y - 25);

            txtXf.Location = new Point(col3, fila2Y);
            MoverControl("lblXf", col3, fila2Y - 25);

            txtH.Location = new Point(col4, fila2Y);
            MoverControl("lblH", col4, fila2Y - 25);
        ...
        private void AplicarEstiloTuani(Control.ControlCollection controles)
        {
            ...
            foreach (Control control in controles)
            {
                ...
                {
                    // 🚀 Borde sutil y fondo blanco puro para las cajas
        ...
        private void ConfigurarEmptyState(string nombreMetodo, string descripcion)
        {
            ...
            pnlDerecha.Controls.Add(new Label { Text = "Cálculo Interno de Pasos:\nn = (x_f - x_0) / h\n\nEuler Simple:\ny_i₊₁ = y_i + h · f(x_i, y_i)\n\nEuler Mejorado (Heun):\nUsa promedio de pendientes para\nreducir el error de truncamiento.", Font = new Font("Consolas", 11, FontStyle.Bold), ForeColor = Color.FromArgb(253, 230, 138), AutoSize = true, Location = new Point(25, 90) });

            Panel pnlNota = new Panel { BackColor = Color.FromArgb(31, 41, 55), Size = new Size(330, 250), Location = new Point(25, 230) };
            pnlNota.Controls.Add(new Panel { BackColor = Color.FromArgb(245, 158, 11), Dock = DockStyle.Left, Width = 4 });
            pnlNota.Controls.Add(new Label { Text = "💡 Teoría del Método:\n\nEuler asume que la pendiente de la curva se mantiene constante en el intervalo 'h'.\n\nEl método de Heun (Mejorado) corrige esta suposición evaluando la derivada al inicio y al final del paso.", Font = new Font("Segoe UI", 10), ForeColor = Color.FromArgb(209, 213, 219), AutoSize = true, MaximumSize = new Size(290, 0), Location = new Point(15, 15) });
